...
int main() {
    ...
    if (!process_image_data) {
        cerr << "Error loading input.png\n";
        stbi_image_free(guide_image_data);
        return 1;
    }

    if (width_I != width_p || height_I != height_p) {
        cerr << "Error: Image dimensions do not match!\n";
        stbi_image_free(guide_image_data);
    ...
    cout << "Applying guided filter on Y..." << endl;
    guided_filter(I_Y, p_grayscale, q_Y, width_I, height_I, r, eps);

    vector<unsigned char> output_image_data(N);
    for (int i = 0; i < N; i++) {
        float Y = q_Y[i];
        output_image_data[i] = static_cast<unsigned char>(max(0.0f, min(255.0f, Y * 255.0f)));
    }

    stbi_write_png("Images/Output/output.png", width_I, height_I, 1, output_image_data.data(), width_I * 1);

    cout << "Guided filter with NEON applied successfully: Images/Output/output.png\n";
