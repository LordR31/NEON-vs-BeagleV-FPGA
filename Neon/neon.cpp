...
// box filter simplu: media locala (cu NEON pe suma locala daca se poate)
...
int main() {
    ...
    // NEON pentru normalizare (pe 4 pixeli simultan)
    for (int i = 0; i < N; i += 4) {
        uint8x8_t g8 = vld1_u8(guide_image_data + i);
        uint8x8_t p8 = vld1_u8(process_image_data + i);

        uint16x8_t g16 = vmovl_u8(g8);
        uint16x8_t p16 = vmovl_u8(p8);

        uint32x4_t g32 = vmovl_u16(vget_low_u16(g16));
        uint32x4_t p32 = vmovl_u16(vget_low_u16(p16));

        float32x4_t g_f = vcvtq_f32_u32(g32);
        float32x4_t p_f = vcvtq_f32_u32(p32);
        float32x4_t scale = vdupq_n_f32(1.0f / 255.0f);

        g_f = vmulq_f32(g_f, scale);
        p_f = vmulq_f32(p_f, scale);

        I_grayscale[i + 0] = (double)vgetq_lane_f32(g_f, 0);
        I_grayscale[i + 1] = (double)vgetq_lane_f32(g_f, 1);
        I_grayscale[i + 2] = (double)vgetq_lane_f32(g_f, 2);
        I_grayscale[i + 3] = (double)vgetq_lane_f32(g_f, 3);

        p_grayscale[i + 0] = (double)vgetq_lane_f32(p_f, 0);
        p_grayscale[i + 1] = (double)vgetq_lane_f32(p_f, 1);
        p_grayscale[i + 2] = (double)vgetq_lane_f32(p_f, 2);
        p_grayscale[i + 3] = (double)vgetq_lane_f32(p_f, 3);
    }
