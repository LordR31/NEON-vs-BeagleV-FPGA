...
void guided_filter_neon(const vector<float>& I, const vector<float>& p, vector<float>& q, int width, int height, int r, float eps) {
    ...
    // Using OpenMP for parallelization of these loops.
    #pragma omp parallel for
    for (int i = 0; i < N; i += 8) {
...
int main() {
    ...
    // Use OpenMP for parallelizing this conversion loop
    #pragma omp parallel for
    for (int i = 0; i < N; i += 8) {
        if (i + 7 < N) {
    ...
    cout << "Applying guided filter on grayscale images (NEON + OpenMP)..." << endl;
    start = clock();
